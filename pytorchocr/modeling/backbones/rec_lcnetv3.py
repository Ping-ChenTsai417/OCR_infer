...
class PPLCNetV3(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            x = F.adaptive_avg_pool2d(x, [1, 40])
            # x = F.avg_pool2d(x, [3, 2]) Original

            # if x.size(2) >= 3:  # Check if height is at least as large as the original kernel height
            #     x = F.avg_pool2d(x, [3, 2])
            # else:
            #     x = F.avg_pool2d(x, [1, 2])  # Adjust kernel size for smaller height
        return x
